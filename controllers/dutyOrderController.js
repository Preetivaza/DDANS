...
export const createDutyOrder = async (req, res) => {
  try {
    const { staff_id, duty_date, description, location } = req.body;

    // Validate required fields
    if (!staff_id || !duty_date || !description || !location) {
    ...
    const notification = new Notification({
      staff_id,
      message: `New duty order at ${location}: ${description}`,
    });
...
const __dirname = path.dirname(__filename);
    ...
    const signatureUrl =
      req.body.signatureUrl || 'https://drive.google.com/uc?export=download&id=1SmxgEV1frtRTcWFz322gSkSnrhMARhiu'; // Default static URL
    ...
    doc.text(`Description: ${dutyOrder.description}`);
    doc.text(`Location: ${dutyOrder.location}`);
...
export const getStaffDuties = async (req, res) => {
  try {
    const staffId = req.user.id; // Ensure this matches the staff's MongoDB ID
    const duties = await DutyOrder.find({ staff_id: staffId })
      .populate('staff_id', 'name email')
      .sort({ duty_date: -1 }); // Latest first
    res.status(200).json(duties);
  } catch (error) {
    res.status(500).json({ message: 'Failed to fetch staff duties' });
  }
};
// Get specific duty assigned to staff
export const getStaffDutyById = async (req, res) => {
  try {
    ...
    if (dutyOrder.staff_id.toString() !== staffId) {
      return res.status(403).json({ message: 'Unauthorized to view this duty' });
    }
...
export const markDutyAsCompleted = async (req, res) => {
  try {
    ...
    if (dutyOrder.staff_id.toString() !== staffId) {
      return res.status(403).json({ message: 'Unauthorized to update this duty' });
    }

    if (!['approved', 'pending'].includes(dutyOrder.status)) {
      return res.status(400).json({ message: 'Can only complete approved/pending duties' });
    }

    // Update status to completed
    dutyOrder.status = 'completed';
    dutyOrder.progress_report = req.body.progress_report || 'Completed successfully';
    await dutyOrder.save();

    // Create audit log
    await AuditLog.create({
      action: 'Duty Marked as Completed',
    ...
    await Notification.create({
      message: `Staff ${dutyOrder.staff_id.name} marked duty #${dutyOrder._id} as completed`,
      recipients: ['admin', 'authority'],
    });

    res.status(200).json({ message: 'Duty marked as completed', dutyOrder });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
